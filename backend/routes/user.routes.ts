...
router
  ...
  .put(requireOwnershipOrRole(Role.ADMIN, Role.SUPER_ADMIN),updateUser)
  .delete(requireRole( Role.SUPER_ADMIN),deleteUser)

router
  .route("/")
  .post(requireRole(Role.SUPER_ADMIN, Role.ADMIN), createUser)
  .get(requireRole(Role.ADMIN, Role.SUPER_ADMIN),getUsers)
